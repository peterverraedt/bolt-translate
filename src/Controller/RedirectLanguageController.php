 ...
 */
class RedirectLanguageController implements ControllerProviderInterface
{
    /** @var Application */
    protected $app;
    ...
    public function connect(Application $app)
    {
        ...
        $ctr->match('/{locale}/{subpath}', [$this, 'callbackLocalized'])
            ->assert('locale', '(nl|en|fr|de)')
            ->assert('subpath', '.*');
     ...
     * @param string  $locale
     */
    public function callbackLocalized(Request $request, $locale, $subpath) 
    {
        dump($locale);
        dump($subpath);

        $uri = $request->getUriForPath('/') . $subpath;
